...
<!--      <xsl:apply-templates select="Workbook/Worksheet[not(@ss:Name='LAB FHIR Mapping')]"/>-->
    </mappings>
	</xsl:template>
    ...
    <xsl:variable name="domains" as="element()+">
      ...
      <domain code="LB" name="Laboratory"/>
              ...
              <xsl:variable name="mappings" as="element()">
                <mapping>
                  ...
                  <xsl:if test="t:hasCellValue(., $pathColumn)">
                    <xsl:attribute name="path" select="t:cellValue(., $pathColumn)"/>
                  </xsl:if>
                </mapping>
              </xsl:variable>
              <xsl:choose>
    ...
    <xsl:variable name="value" select="$node/Cell[position()=$column]/Data"/>
